...
def validate_password(password: str):
    ...
    if password.lower() in simple: return "Пароль слишком простой, придумай другой"
    return None
...
def like_post(post_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if existing:
        ...
        if post and post.user_id != user.id:
            notification = models.Notification(user_id=post.user_id, message=f"{user.username} лайкнул ваш пост")
            db.add(notification)
    db.commit()
    return RedirectResponse("/", status_code=302)
...
def profile(username: str, request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "profile.html", {"user": current_user, "profile_user": profile_user, "posts": posts, "is_following": is_following})
...
def messages_page(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "messages.html", {"user": user, "conversations": conversations})
...
def conversation(username: str, request: Request, db: Session = Depends(get_db)):
    ...
    for msg in msgs:
        if msg.receiver_id == user.id and not msg.is_read: msg.is_read = True
    db.commit()
    return templates.TemplateResponse(request, "conversation.html", {"user": user, "other": other, "messages": msgs})
...
def search(request: Request, q: str = "", db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "search.html", {"user": user, "results": results, "q": q})
...
def settings_save(request: Request, name: str = Form(...), bio: str = Form(""), username: str = Form(...), db: Session = Depends(get_db)):
    ...
    if not user: return RedirectResponse("/login", status_code=302)
    error = validate_username(username)
    if error: return templates.TemplateResponse(request, "settings.html", {"user": user, "error": error})
    existing = db.query(models.User).filter(models.User.username == username, models.User.id != user.id).first()
    if existing: return templates.TemplateResponse(request, "settings.html", {"user": user, "error": f"Никнейм @{username} занят"})
    user.name, user.bio, user.username = name, bio, username
    db.commit()
    token = auth.create_token({"sub": username})
...
def change_password(request: Request, old_password: str = Form(...), new_password: str = Form(...), db: Session = Depends(get_db)):
    ...
    db.commit()
    return templates.TemplateResponse(request, "settings.html", {"user": user, "success": "Пароль изменён"})
